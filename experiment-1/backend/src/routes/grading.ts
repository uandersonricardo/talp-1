...
gradingRouter.post(
  "/",
  upload.fields([{ name: "answers" }, { name: "responses" }]),
  (req: Request, res: Response) => {
    try {
      ...
      if (!answersFile) return res.status(400).json({ error: "answers file is required" });
      if (!responsesFile) return res.status(400).json({ error: "responses file is required" });
